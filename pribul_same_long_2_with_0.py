...
pribul_bufer = 0
old_pribul =0
for i in range(1300, 1500):  # while (ik < 1): # количество файлов
    # ik = ik + 1
    ...
    while (steps < len(viborka)):
        if old_pribul<0:
            freez = old_pribul
            n_k_st = math.fabs(round(old_pribul/0.12))
            print('n_k_st:', n_k_st)
            stavka_pribuli = 0.24 *n_k_st
            stavka_ubuli = 0.12 *n_k_st
            print('stavka_ubuli: ', stavka_ubuli)
            old_pribul = 0
        key = viborka[steps]
        # if pribul_bufer< 0:
        #    pribul_same = pribul_bufer
        key1 = key
        steps = steps + 1
        if old_key > -1:
            ...
            if ((old_key == 1) or (old_key == 4) or (old_key == 7) or (old_key == 10) or (old_key == 13) or (
                ...
                freez = pribul_same
                pribul_same = pribul_same - stavka_ubuli
                # glob_pribul_same = pribul_same
                # stavka_pribuli = stavka_pribuli *2
                # stavka_ubuli = stavka_ubuli *2
                print('stavka_ubuli:', stavka_ubuli)
            if ((old_key == 2) or (old_key == 5) or (old_key == 8) or (old_key == 11) or (old_key == 14) or (
                old_key == 17) or (old_key == 20) or (old_key == 23) or (old_key == 26) or (old_key == 29) or (
                old_key == 32) or (old_key == 35)) \
        ...
        if pribul_same> freez:
            stavka_pribuli = 0.24
            stavka_ubuli = 0.12
            print('staavkaa_ ubuli: ',stavka_ubuli)
            # glob_pribul_same =0
        # if pribul_same < 0:
        #     stavka_pribuli = stavka_pribuli+ 0.24
        #     stavka_ubuli = stavka_ubuli+0.12
